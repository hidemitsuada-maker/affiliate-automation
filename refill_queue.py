...
SYSTEM = (
    ...
    "検索ボリュームが見込め、かつ提携ASPの否認条件を踏まない読者層に当たる"
    "ロングテールKWを設計します。既存記事と共食い(カニバリ)しないテーマを選びます。"
)
...
- slugは内容を表す英語(例: inhouse-se-night-shift)。既存slugと重複させない。
- keywordは実際に検索されるロングテール(2〜4語)。読者層制約を踏む層を狙わない。
